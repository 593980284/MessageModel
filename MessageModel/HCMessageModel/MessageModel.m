...
@property(nonatomic, strong) NSInvocation * inv;
@property(nonatomic, strong) NSMethodSignature * sig;
@end
@implementation MessageModel
- (instancetype)initWithTarget:(id) target
                           sel:(SEL) sel,...{
    NSAssert(target,@"target不能为nil");
    NSAssert(sel,@"sel不能为nil");
    
    if (self = [super init]) {
        NSMethodSignature * sig = [target methodSignatureForSelector:sel];
        if (!sig) { [target doesNotRecognizeSelector:sel]; return nil; }
        NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
        if (!inv) { [target doesNotRecognizeSelector:sel]; return nil; }
        [inv setTarget:target];
        [inv setSelector:sel];
        va_list args;
        va_start(args, sel);
        [self setInv:inv withSig:sig andArgs:args];
        va_end(args);
        _inv = inv;
        _sig = sig;
        _target = target;
...
- (id)msgSend{
    [_inv invoke];
    return [self getReturnFromInv:_inv withSig:_sig];
}
